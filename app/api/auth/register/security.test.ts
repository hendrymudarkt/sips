...
import {
  apiRateLimiter } from '@/lib/auth/rateLimiter';

import {
  validateCsrfToken } from '@/lib/auth/csrf';
...
describe('Register API Security', () => {
  ...
  vi.mocked(apiRateLimiter.consume).mockResolvedValue({
    ...
    vi.mocked(cookies).mockResolvedValue({
  get: vi.fn().mockReturnValue({
  value: 'valid-csrf' }),    } as never);

    vi.mocked(validateCsrfToken).mockReturnValue(true);

    vi.mocked(global.fetch).mockResolvedValue({
  ok: false,      status: 500,      json: async () => ({
  message: 'Detailed SQL error: table users not found' }),      text: async () => JSON.stringify({
  message: 'Detailed SQL error: table users not found' }),    } as Response);
    const req = new NextRequest('http://localhost/api/register', {
