...
describe('Force Logout API Security', () => {
  ...
  it('should clear cookies and return ok on success', async () => {
    ...
    const mockCookieStore = {
      delete: vi.fn(),
      get: vi.fn().mockReturnValue({ value: 'some-csrf-token' }),
    };
    const { cookies } = await import('next/headers');
    vi.mocked(cookies).mockResolvedValue(mockCookieStore as never);
    const req = new NextRequest('http://localhost/api/auth/force-logout', {
      method: 'POST',
    ...
    expect(data.message).toContain('All cookies cleared successfully');
  });
