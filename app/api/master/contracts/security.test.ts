...
describe('Contracts API Security', () => {
  ...
  it('should return 401 if no token', async () => {
    ...
    const req = new NextRequest('http://localhost/api/master/contracts');
  ...
  it('should return generic error message on upstream failure', async () => {
    vi.mocked(global.fetch).mockResolvedValue({
      ok: false,
      status: 500,
      text: async () => JSON.stringify({ message: 'DB connection failed at 10.0.0.5:5432' }),
      json: async () => ({ message: 'DB connection failed at 10.0.0.5:5432' }),
    } as Response);
    const req = new NextRequest('http://localhost/api/master/contracts');
    const res = await GET(req);
    expect(res.status).toBe(500);
    const data = await res.json();
    expect(data.error).toBe('Failed to fetch contracts data');
  });
});