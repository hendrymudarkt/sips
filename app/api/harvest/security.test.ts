...
vi.mock('@/utils/api/upstreamProxy', () => ({
  ...
  buildFilteredUrl: vi.fn((base) => base),
}));
...
describe('Harvest API Security', () => {
  ...
  describe('POST handler', () => {
    it('should return security error if validateSecurity fails', async () => {
      ...
      const req = new NextRequest('http://localhost/api/harvest', { method: 'POST' });
    ...
    it('should return generic error message on upstream failure', async () => {
      ...
      vi.mocked(global.fetch).mockResolvedValue({
        ok: false,
        status: 500,
        text: async () => JSON.stringify({ message: 'Detailed SQL error: table panens not found' }),
        json: async () => ({ message: 'Detailed SQL error: table panens not found' }),
      } as Response);
      const formData = new FormData();
      const req = new NextRequest('http://localhost/api/harvest', {
        method: 'POST',
        body: formData,
      });
      const res = await POST(req);
      ...
      const data = await res.json();
      expect(data.error).toBe('Harvest submission failed');
    });
  });
  ...
  describe('GET handler', () => {
    ...
    it('should return generic error message on upstream failure', async () => {
      vi.mocked(global.fetch).mockResolvedValue({
        ok: false,
        status: 500,
        text: async () => JSON.stringify({ message: 'Detailed error: DB crash at 10.0.0.1' }),
        json: async () => ({ message: 'Detailed error: DB crash at 10.0.0.1' }),
      } as Response);
      const req = new NextRequest('http://localhost/api/harvest');
      const res = await GET(req);
      expect(res.status).toBe(500);
      const data = await res.json();
      expect(data.error).toBe('Failed to fetch harvest data');
    });
  });
});
