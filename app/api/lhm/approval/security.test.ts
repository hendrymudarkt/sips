...
describe('LHM Approval API Security', () => {
  beforeEach(() => {
  vi.clearAllMocks();  });
  it('should return 401 if no token', async () => {    const { getTokenFromCookie } = await import('@/utils/api/upstreamProxy');
    vi.mocked(getTokenFromCookie).mockResolvedValue(undefined);    const req = new NextRequest('http://localhost/api/lhm/approval');    const res = await GET(req);    expect(res.status).toBe(401);  });
  it('should call proxyGet when token is present', async () => {    const { proxyGet } = await import('@/lib/api/apiProxy');
    vi.mocked(proxyGet).mockResolvedValue(      new Response(JSON.stringify({ success: true, data: [] })) as never    );    const req = new NextRequest('http://localhost/api/lhm/approval');    await GET(req);    expect(proxyGet).toHaveBeenCalled();  });});