...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || data.message || tAuth('resetError'));
        setIsLoading(false);
        return;
      }
        ...
        <div className="card bg-base-100 card-border border-base-300 w-full max-w-sm overflow-hidden shadow-lg shadow-base-300/40 animate-fadeIn [animation-duration:600ms]">
              ...
              <div className="grow flex items-center gap-2">
                <div className="flex flex-col">
                  <span className="text-xs font-semibold tracking-[0.22em] text-primary/80">
                    {tAuth('forgotPassword').toUpperCase()}
                  </span>
          ...
          {sent ? (
            <div className="card-body gap-4">
              <div className="flex items-center gap-3 rounded-2xl bg-success/10 border border-success/20 px-4 py-3 animate-fadeIn [animation-duration:300ms]">
                <Icon name="check" className="h-5 w-5 text-success shrink-0" />
            ...
            <form onSubmit={handleSubmit} className="card-body gap-4">
              ...
              <button
                ...
                disabled={isLoading}
                aria-label={isLoading ? tAuth('sending') : tAuth('sendResetLink')}
              >
                {isLoading ? (
                  <>
