...
describe('perf-formatter', () => {
  ...
  describe('perfCompare', () => {
    it('should compare strings correctly', () => {
      expect(perfCompare('apple', 'banana', 'id-ID')).toBeLessThan(0);
      expect(perfCompare('banana', 'apple', 'id-ID')).toBeGreaterThan(0);
      expect(perfCompare('apple', 'apple', 'id-ID')).toBe(0);
    });

    it('should support sorting with cached collator', () => {
      const arr = ['cherry', 'banana', 'apple'];
      arr.sort((a, b) => perfCompare(a, b, 'id-ID'));
